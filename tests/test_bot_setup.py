...
def _patch_run_setup(
    *,
    ...
    """
    Return a dict of patch targets pre-filled with sensible defaults.
    Callers can override individual values before entering the context.
    """
    config = config or _base_config()
    men_games = men_games if men_games is not None else []
    women_games = women_games if women_games is not None else []
    top_men = top_men if top_men is not None else ["Alice (120 pts)", "Bob (110 pts)"]
    top_women = top_women if top_women is not None else ["Carol (115 pts)"]
    summary_blocks = summary_blocks if summary_blocks is not None else (
        [{"type": "section", "text": {"type": "mrkdwn", "text": "summary"}}],
        False,  # no_games=False
    )

    return {
        "get_input_safe": method_input,
        "ask_if_missing": config,
        "ask_slack_credentials_cli": config,
        "save_json": MagicMock(),
        "get_final_games_men": men_games,
        "get_final_games_women": women_games,
        "build_daily_summary": summary_blocks,
        "build_yearly_intro_message": "🏆 Intro message",
        "post_message": MagicMock(),
        "load_flag": {"LIVE_FOR_YEAR": False},
        "deduplicate_top_users_men": top_men,
        "deduplicate_top_users_women": top_women,
...
def test_clean_url_plain():
    from bot_setup import _clean_url
...
def test_tournament_end_reads_from_config_men():
    from bot_setup import _tournament_end
...
def test_tournament_end_falls_back_to_default_men():
    from bot_setup import _tournament_end, _DEFAULT_TOURNAMENT_END_MEN
    assert _tournament_end({}, "men's") == _DEFAULT_TOURNAMENT_END_MEN


def test_tournament_end_falls_back_to_default_women():
    from bot_setup import _tournament_end, _DEFAULT_TOURNAMENT_END_WOMEN
...
def test_schedule_incomplete_config_reminder_writes_flag(tmp_path):
    from bot_setup import schedule_incomplete_config_reminder, INCOMPLETE_CONFIG_FLAG
    with patch("bot_setup.INCOMPLETE_CONFIG_FLAG", tmp_path / "incomplete_config.json"):
        with patch("bot_setup.next_weekday_morning", return_value=datetime.datetime(2026, 3, 16, 9, 0)):
            schedule_incomplete_config_reminder()
            flag_path = tmp_path / "incomplete_config.json"
...
def test_check_incomplete_config_reminder_no_file_is_noop(tmp_path, capsys):
    from bot_setup import check_incomplete_config_reminder
    with patch("bot_setup.INCOMPLETE_CONFIG_FLAG", tmp_path / "missing.json"):
        check_incomplete_config_reminder()
    assert capsys.readouterr().out == ""


def test_check_incomplete_config_reminder_prints_when_overdue(tmp_path, capsys):
    from bot_setup import check_incomplete_config_reminder  # add import
    ...
    with patch("bot_setup.INCOMPLETE_CONFIG_FLAG", flag):
        with patch("bot_setup.next_weekday_morning", return_value=datetime.datetime(2026, 3, 17, 9, 0)):
            check_incomplete_config_reminder()
...
def test_clear_incomplete_config_reminder_removes_file(tmp_path):
    ...
    flag.write_text("{}")
    with patch("bot_setup.INCOMPLETE_CONFIG_FLAG", flag):
        from bot_setup import clear_incomplete_config_reminder
...
# ---------------------------------------------------------------------------
# run_setup — CLI path, no bracket URLs, go-live confirmed
# ---------------------------------------------------------------------------

@patch("bot_setup.save_json")
@patch("bot_setup.post_message")
@patch("bot_setup.build_yearly_intro_message", return_value="intro")
@patch("bot_setup.build_daily_summary", return_value=([{"type": "section", "text": {"type": "mrkdwn", "text": "s"}}], False))
...
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.get_input_safe", side_effect=["cli", "", "", "", "y"])  # extra "" for any mid-flow prompt
...
@patch("bot_setup.load_flag", return_value={"LIVE_FOR_YEAR": False})
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.get_input_safe", side_effect=["cli", "", "", "", "n"])
def test_run_setup_cli_go_live_declined_does_not_post(
    mock_input, mock_ask_if_missing, mock_load_flag,
    mock_games, mock_dedup, mock_summary, mock_intro,
    mock_post, mock_save
):
...
@patch("bot_setup.load_flag", return_value={"LIVE_FOR_YEAR": False})
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.get_input_safe", side_effect=["garbage", "", "", "", "n"])
...
# ---------------------------------------------------------------------------
# run_setup — missing POOLS after setup returns early
# ---------------------------------------------------------------------------

@patch("bot_setup.save_json")
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.get_input_safe", return_value="cli")
def test_run_setup_missing_pools_returns_early(mock_input, mock_ask, mock_save, capsys):
    from bot_setup import run_setup
    config_in = {"METHOD": "cli", "POOLS": []}  # empty POOLS list — guard should fire

    with patch("bot_setup.get_final_games", return_value=[]):
        result = run_setup(config_in)

    out = capsys.readouterr().out
    assert "[ERROR]" in out
    # Should return empty game lists
...
@patch("bot_setup.build_daily_summary", return_value=([{"type": "section", "text": {"type": "mrkdwn", "text": "s"}}], False))
@patch("bot_setup.deduplicate_top_users", side_effect=lambda x: x)
@patch("bot_setup.get_final_games", side_effect=[
    [{"id": "m1", "home": "A", "away": "B", "home_score": 70, "away_score": 65}],
    [{"id": "w1", "home": "C", "away": "D", "home_score": 80, "away_score": 75}],
])
@patch("bot_setup.load_flag", return_value={"LIVE_FOR_YEAR": False})
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.get_input_safe", side_effect=["cli", "", "", "", "y"])
def test_run_setup_writes_seen_file_with_game_ids(
    mock_input, mock_ask, mock_load_flag,
    mock_games, mock_dedup, mock_summary, mock_intro,
    mock_post, mock_save
):
    from bot_setup import run_setup, SEEN_FILE
    config_in = _base_config()

    run_setup(config_in)

    seen_call = next(
        (c for c in mock_save.call_args_list if c.args[0] == SEEN_FILE),
        None
    )
...
@patch("bot_setup.post_message")
@patch("bot_setup.build_yearly_intro_message", return_value="intro")
@patch("bot_setup.build_daily_summary", return_value=([{"type": "section", "text": {"type": "mrkdwn", "text": "s"}}], True))
@patch("bot_setup.deduplicate_top_users", side_effect=lambda x: x)
@patch("bot_setup.get_final_games", return_value=[])
@patch("bot_setup.load_flag", return_value={"LIVE_FOR_YEAR": False})
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
...
def test_run_setup_off_day_skips_summary_post(
    ...
    # intro message should still be posted, but blocks (summary) should not
    calls = mock_post.call_args_list
    assert any(c.kwargs.get("text") == "intro" or (c.args[1:2] and c.args[1] == "intro") for c in calls), \
        "intro message should be posted"
...
@patch("bot_setup.schedule_incomplete_config_reminder")
@patch("bot_setup.save_json")
@patch("bot_setup.run_slack_dm_setup", return_value=None)
@patch("bot_setup.ask_slack_credentials_cli", side_effect=lambda c: {
    **c,
    "SLACK_WEBHOOK_URL": "https://hooks.slack.com/TEST",
    "SLACK_MANAGER_ID": "U123",
})
@patch("bot_setup.get_input_safe", return_value="slack")
def test_run_setup_slack_dm_timeout_returns_none_config(
    mock_input, mock_creds, mock_dm_setup, mock_save, mock_reminder
):
    from bot_setup import run_setup
    result = run_setup({})
    config_out, method, men, women, top_m, top_w = result
...
@patch("bot_setup.load_flag", return_value={"LIVE_FOR_YEAR": False})
@patch("bot_setup.ask_if_missing", side_effect=lambda config, key, *a, **kw: config)
@patch("bot_setup.ask_slack_credentials_cli", side_effect=lambda c: c)  # returns config unchanged (no creds)
@patch("bot_setup.get_input_safe", side_effect=["slack", "", "", "", "n"])
...
def test_fetch_leaderboard_exception_asks_manual_in_slack_mode(mock_get_top, mock_run_async):
    ...
    with patch("bot_setup.deduplicate_top_users", side_effect=lambda x: x):
        with patch("slack_dm.send_dm", return_value=("C123", "ts1")):
            with patch("slack_dm.ask_manual_top_users", return_value=["Manual User (50 pts)"]) as mock_manual:
...
def test_run_setup_posts_in_mock_mode_when_no_webhook(
    ...
    from bot_setup import run_setup
    config_in = _base_config()
    config_in["SLACK_WEBHOOK_URL"] = ""  # no webhook → mock=True
